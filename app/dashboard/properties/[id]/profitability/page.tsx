...
const fmtEur = (n: number, showSign = false, dash = false) => {
  if (dash && n === 0) return <span className="text-zinc-300 dark:text-zinc-600">—</span>;
      ...
      <section className="mx-auto w-full max-w-7xl space-y-6">
        ...
        <div>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <select
              ...
              className="w-fit rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-900 shadow-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
            >
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <SummaryCard
            ...
            color={yearTotals.steuerlicher_gewinn_verlust <= 0 ? "emerald" : "zinc"}
            ...
            <table className="w-full text-sm">
              <thead>
                ...
                <tr className="border-b border-zinc-100 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-800/60">
                  <th className="whitespace-nowrap px-4 py-2 text-left text-xs font-semibold text-zinc-400 dark:text-zinc-500" rowSpan={2}>
                    Monat
                  </th>
                  <th className="border-l border-zinc-100 px-4 py-2 text-center text-xs font-semibold text-emerald-600 dark:border-zinc-800 dark:text-emerald-400">
                    Einnahmen
                  </th>
                  {visibleCols.length > 0 && (
                    <th
                      colSpan={visibleCols.length + 1}
                      className="border-l border-zinc-100 px-4 py-2 text-center text-xs font-semibold text-red-500 dark:border-zinc-800 dark:text-red-400"
                    >
                      Ausgaben
                    </th>
                  )}
                  <th className="border-l border-zinc-100 px-4 py-2 text-center text-xs font-semibold text-zinc-600 dark:border-zinc-800 dark:text-zinc-300" colSpan={2}>
                    Ergebnis
                  </th>
                </tr>
                <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800/60">
                  {/* Einnahmen */}
                  <th className="border-l border-zinc-100 whitespace-nowrap px-4 py-2 text-right text-xs font-medium text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                    Gesamt
                  </th>
                  {/* Ausgaben-Spalten */}
                  {visibleCols.map((col) => (
                    <th key={col.key} className="border-l border-zinc-100 whitespace-nowrap px-4 py-2 text-right text-xs font-medium text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                      {col.label}
                      {col.hint && (
                        <span className="ml-1 font-normal text-zinc-400 dark:text-zinc-500">{col.hint}</span>
                      )}
                    </th>
                  ))}
                  {/* Ausgaben-Gesamt */}
                  ...
                  {/* Cashflow + AfA */}
                  <th className="border-l border-zinc-200 whitespace-nowrap px-4 py-2 text-right text-xs font-medium text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">
                    Cashflow
                  </th>
                  <th className="whitespace-nowrap px-4 py-2 text-right text-xs font-medium text-zinc-500 dark:text-zinc-400">
                    AfA (Monat)
                  </th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {months.map(({ m, label, einnahmen, colValues, ausgabenTotal, cashflow, hasTx }) => (
                  ...
                  >
                    ...
                    {/* Einnahmen */}
                    <td className="border-l border-zinc-100 px-4 py-2.5 text-right text-sm tabular-nums dark:border-zinc-800">
                      <span className={einnahmen > 0 ? "font-medium text-emerald-600 dark:text-emerald-400" : "text-zinc-300 dark:text-zinc-600"}>
                    ...
                    {visibleCols.map((col) => {
                      ...
                      return (
                        <td key={col.key} className="border-l border-zinc-100 px-4 py-2.5 text-right text-sm tabular-nums text-zinc-600 dark:border-zinc-800 dark:text-zinc-400">
                          {fmtEur(v, false, true)}
                        </td>
                      );
                    })}
                    {/* Ausgaben Gesamt */}
                    <td className="border-l border-zinc-100 px-4 py-2.5 text-right text-sm tabular-nums dark:border-zinc-800">
                      <span className={ausgabenTotal > 0 ? "font-medium text-red-600 dark:text-red-400" : "text-zinc-300 dark:text-zinc-600"}>
                        {ausgabenTotal > 0 ? fmtEur(ausgabenTotal) : "—"}
                      </span>
                    </td>
                    {/* Cashflow */}
                    <td className={`border-l border-zinc-200 px-4 py-2.5 text-right text-sm font-semibold tabular-nums dark:border-zinc-700 ${
                      !hasTx        ? "text-zinc-300 dark:text-zinc-600"
                    ...
                    {/* AfA */}
                    <td className="px-4 py-2.5 text-right text-sm tabular-nums text-zinc-400 dark:text-zinc-500">
              ...
              {/* ── Jahressummen ───────────────────────────────────────────── */}
              <tfoot>
                <tr className="border-t-2 border-zinc-200 bg-zinc-50 font-semibold dark:border-zinc-700 dark:bg-zinc-800/60">
                  <td className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                    Gesamt {year}
                  </td>
                  {/* Einnahmen */}
                  <td className="border-l border-zinc-200 px-4 py-3 text-right text-sm font-bold tabular-nums text-emerald-600 dark:border-zinc-700 dark:text-emerald-400">
                    {fmtEur(yearTotals.einnahmen)}
                  </td>
                  {/* Ausgaben-Spalten */}
                  {visibleCols.map(({ key }) => (
                    <td key={key} className="border-l border-zinc-200 px-4 py-3 text-right text-sm tabular-nums text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">
                      {yearTotals.colTotals[key] > 0 ? fmtEur(yearTotals.colTotals[key]) : "—"}
                    </td>
                  ))}
                  {/* Ausgaben Gesamt */}
                  <td className="border-l border-zinc-200 px-4 py-3 text-right text-sm font-bold tabular-nums text-red-600 dark:border-zinc-700 dark:text-red-400">
                    {fmtEur(yearTotals.ausgaben)}
                  </td>
                  {/* Cashflow */}
                  <td className={`border-l border-zinc-300 px-4 py-3 text-right text-sm font-bold tabular-nums dark:border-zinc-600 ${
                  ...
                  {/* AfA */}
                  <td className="px-4 py-3 text-right text-sm tabular-nums text-zinc-500 dark:text-zinc-400">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="rounded-b-xl bg-zinc-50 px-5 py-3 dark:bg-zinc-800/50">
        ...
        {propertyInput.kaufpreis > 0 && (
          <div className="rounded-xl border border-zinc-200 bg-white px-5 py-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <h2 className="mb-3 text-sm font-semibold text-zinc-700 dark:text-zinc-300">AfA-Grundlagen</h2>
...
function TaxRow({
  label, value, valueClass = "text-zinc-700 dark:text-zinc-300", hint, indent,
}: {
  ...
  return (
    <div className={`flex items-start justify-between gap-4 py-2.5 ${indent ? "pl-4" : ""}`}>
      <div>
        <p className={`text-sm ${indent ? "text-zinc-500 dark:text-zinc-400" : "text-zinc-700 dark:text-zinc-300"}`}>{label}</p>
        {hint && <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-500">{hint}</p>}
      </div>
...
function AfAItem({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="py-1">
      ...
      <p className="mt-0.5 font-medium text-zinc-800 dark:text-zinc-200">{value}</p>
    </div>
  );
}
